...
if __name__ == '__main__':
	...
	loss_v = train_NN(X_train, Y_train, a_func, S1, S2, S3, learning_rate, lmda, iterations)
	...
	for i in range(500):
		output_ = FWP(X_train[:,i], Y_train[:,i], params, a_func)
		print(output_['a3'])

	# print out the activations of the last layer on testing data
	for i in range(140):
		output_ = FWP(X_test[:,i], Y_test[:,i], params, a_func)
		print(output_['a3'])

	# testing NN on test data set
	print('The classification error on test data is ', testing_error(X_test, Y_test, params, a_func))
